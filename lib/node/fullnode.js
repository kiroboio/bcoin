...
const UtxoIndexer = require('../indexer/utxoindexer');
...
class FullNode extends Node {
  ...
  constructor(options) {
    ...
    if (this.config.bool('index-utxo')) {
      this.utxoindex= new UtxoIndexer({
        network: this.network,
        logger: this.logger,
        blocks: this.blocks,
        chain: this.chain,
  ...
  init() {
    ...
    if (this.utxoindex)
      this.utxoindex.on('error', err => this.error(err));
  ...
  async getUtxoByAddress(addr, options = {}) {
    if (!this.txindex || !this.utxoindex)
      return [];
    ...
    const confirmed = async () => {
      const hashes = await this.utxoindex.getHashesByAddress(
        addr, {limit, reverse, after});
