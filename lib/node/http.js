...
class HTTP extends Server {
  ...
  handleAuth(socket) {
    ...
    socket.hook('get tx', async (...args) => {
      const valid = new Validator(args[0]);
      ...
      return meta.getJSON(this.network, view, this.chain.height);
    });

    socket.hook('get txByAddress', async (...args) => {
      const valid = new Validator(args[0]);
      const address = valid.str('address');
      const limit = valid.uint('limit', this.options.maxTxs);
      const reverse = valid.bool('reverse', false);
    ...
    socket.hook('get utxo', async (...args) => {
      const valid = new Validator(args[0]);
      ...
      enforce(hash, 'txid is required.');
      enforce(index != null, 'vout is required.');
      enforce(!this.chain.options.spv, 'Cannot get coins in SPV mode.');
  ...
  initSockets() {
    ...
    pool.on('add entry', (entry) => {
      ...
      for (const socket of sockets) {
        if (!this.filterTX(socket, entry.tx))
          continue;

        socket.fire('add entry', raw);
      }
    });

    pool.on('tx remove', (tx) => {
      const sockets = this.channel('mempool');
      ...
      for (const socket of sockets) {
        ...
        socket.fire('tx remove', raw);
      }
    });

    pool.on('conflict', (entry, exists) => {
      const sockets = this.channel('mempool');
      ...
      for (const socket of sockets) {
        ...
        const raw = entry.toRaw();
        socket.fire('conflict', raw, exists);
      }
    });
