 ...
 //       tx.hash());
  ...
  async insertTX(tx, id) {
    ...
    if (this.isDoubleSpend(tx)) {
      ...
      const doubleSpenders = this.getDoubleSpenders(tx)
      for (const doubleSpender of doubleSpenders) {
        this.emit('conflict', doubleSpender);
        if (this.grade(doubleSpender) >= this.grade(entry)) {
          throw new VerifyError(tx,
            'duplicate',
            'bad-txns-inputs-spent',
          0);
      ...
      for (const doubleSpend of doubleSpends) {
        this.evictEntry(doubleSpend);
      }
    }
...
//      tx.hash(), this.map.size);
  ...
  getDoubleSpenders(tx) {
    const res = {}
    for (const {prevout} of tx.inputs) {
      const {hash, index} = prevout;
      const entry = this.getSpent(hash, index);
      if (entry) {
        res[entry.hash().toString('hex')] = entry   
      }
    }
