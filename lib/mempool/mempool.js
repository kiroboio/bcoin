 ...
 //       tx.hash());
  ...
  async insertTX(tx, id) {
    ...
    if (this.isDoubleSpend(tx)) {
      ...
      console.log('CONFLICT2', this.grade(entry))
      const doubleSpends = this.getDoubleSpend(tx)
      const grade = entry.fee / entry.size
      for (const doubleSpend of doubleSpends) {
        this.emit('conflict', doubleSpend);
        if (this.grade(doubleSpend) >= this.grade(entry)) {
          throw new VerifyError(tx,
            'duplicate',
            'bad-txns-inputs-spent',
          0);
...
//      tx.hash(), this.map.size);
  ...
  getDoubleSpend(tx) {
    ...
    for (const {prevout} of tx.inputs) {
      const {hash, index} = prevout;
      if (this.isSpent(hash, index))
        res.push(this.getSpent(hash, index));
    }
