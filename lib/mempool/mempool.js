...
class Mempool extends EventEmitter {
 ...
 //       tx.hash());
  ...
  grade(entry) {
    const fee = entry.fee >=0 ? entry.fee : 1;
    if (entry.rbf) {
      // TODO: use _getAncestors and __countAncestors to match bitcoin core and bip-125
      return fee;
    }
    const size = entry.size >= 0 ? entry.size : 1;
    return fee / size;
  }
  ...
  async insertTX(tx, id) {
    ...
    if (this.isDoubleSpend(tx)) {
      ...
      let entry;
      if (fee < 0) {
        const spentView = await this._getSpentView(tx);
        entry = MempoolEntry.fromTX(tx, spentView, height, rbf);
      } else {
        entry = MempoolEntry.fromTX(tx, view, height, rbf);
      }
      const doubleSpents = this.getDoubleSpents(tx);
      let replace = true;
      for (const doubleSpent of doubleSpents) {
        if (this.grade(doubleSpent) >= this.grade(entry)) {
          replace = false;
        }
      }
      for (const doubleSpent of doubleSpents) {
          if (replace) {
            if (!rbf) {
              this.emit('conflict', doubleSpent, false);
              this.emitEventToSpenders(doubleSpent, 'conflict', false);
              this.emit('conflict', entry, true);
            }
            this.evictEntry(doubleSpent);
          } else {
            if (!rbf) {
              this.emit('conflict', doubleSpent, true);
              this.emitEventToSpenders(doubleSpent, 'conflict', true);
              this.emit('conflict', entry, false);
            }
          }
      }
    ...
    const entry = MempoolEntry.fromTX(tx, view, height, rbf);
    ...
    // if (this.isDoubleSpend(tx)) {
    //   this.emit('conflict', entry);
    //   const doubleSpents = Object.values(this.getDoubleSpents(tx))
    //   for (const doubleSpent of doubleSpents) {
    //     this.emit('conflict', doubleSpent);
    //     if (this.grade(doubleSpent) >= this.grade(entry)) {
    //       throw new VerifyError(tx,
    //         'duplicate',
    //         'bad-txns-inputs-spent',
...
//    this.logger.debug(
//      'Removed %h from mempool (txs=%d).',
//      tx.hash(), this.map.size);

    this.emit('tx remove', tx);
  ...
  emitEventToSpenders(entry, event, wins) {
    const tx = entry.tx;
    const hash = tx.hash();
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      this.emitEventToSpenders(spender, wins);
      this.emit(event, spender, wins)
    }
  }
  ...
  getDoubleSpents(tx) {
    ...
    for (const {prevout} of tx.inputs) {
      const {hash, index} = prevout;
      const entry = this.getSpent(hash, index);
      if (set.has(entry.hash()))
        continue;

      set.add(entry.hash());
      res.push(entry);
      // this._getDescendants(entry, res, set);
    }
  ...
  trackEntry(entry, view) {
    ...
    if (tx.isRBF()) {
      this.rbfs.set(hash, entry);
    } else for (const {prevout} of tx.inputs) {
      if (this.rbfs.has(prevout.hash)) {
        this.rbfs.set(hash);
        break;
      }
