/*!
 * utxoindexer.js - address indexer for bcoin
...
/*
 * UtxoIndexer Database Layout:
 *  A[addr-prefix][addr-hash][height][index][txout] -> dummy (tx by address)
 *  B[addr-prefix][addr-hash] -> balance
...
Object.assign(layout, {
  A: bdb.key('A', ['uint8', 'hash', 'uint32', 'uint32', 'uint16']),
  B: bdb.key('B', ['uint8', 'hash']),
...
/**
 * UtxoIndexer
 * @alias module:indexer.UtxoIndexer
 * @extends Indexer
 */

class UtxoIndexer extends Indexer {
  /**
   * Create a indexer
   * @constructor
  ...
  async indexBlock(meta, block, view) {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      for (let txout = 0; txout < tx.outputs.length; ++txout) {
	  //console.log('indexing txout:', txout);
	const output = tx.outputs[txout];
	...
	console.log(`put(${addr}, ${height}, ${i}, ${txout})`);
        this.put(layout.A.encode(prefix, addrHash, height, i, txout), null);

        hasAddress = true;
      }
      for (const input of tx.inputs) {
	      const inAddr = input.getAddress();
	      if (!inAddr)
		      continue;

	      const inPrefix = inAddr.getPrefix(this.network);

	      if (inPrefix < 0)
		      continue;

	      const inAddrHash = inAddr.getHash();
	      const { prevout } = input;
	     
  	      const raw = await this.db.get(layout.c.encode(prevout.hash));
	      if (raw) {
              	const count = Count.fromRaw(raw);
	    	console.log(`del(${inAddr}, ${count.height}, ${count.index}, ${prevout.index})`);
  	        this.del(layout.A.encode(inPrefix, inAddrHash, count.height, count.index, prevout.index));
	      }
	      //console.log(prevout.hash, prevout.index);
      }
  ...
  async unindexBlock(meta, block, view) {
    const height = meta.height;
	  console.log('unindexing block:', height);
  ...
  async getHashesByAddress(addr, options = {}) {
    ...
    const opts = {
      ...
      reverse,
      parse: (key) => {
        const [,, height, index, utxo] = layout.A.decode(key);
        return [height, index, utxo];
      }
    };
